...
走完前七章，模型改造态的整体图谱可以这样收一下。

## 8.1 改造态全景：从 SFT 到对齐到安全

改造态本质是一条"模型工厂"流水线。每个base model出来后要走完整链路才能上线：

| 阶段 | 输入 | 输出 | 典型方法 | 章节 |
|---|---|---|---|---|
| SFT | base model + 高质量"prompt-回答"对 | 能回答问题的对齐模型 | supervised fine-tuning | 2.2 |
| 偏好对齐 | SFT模型 + 人类/AI偏好数据 | 对齐到HHH的产品模型 | RLHF（PPO）或DPO | 三、四 |
| 合成数据扩展 | 强模型生成 | 扩大训练数据规模 | Constitutional AI、RLAIF、蒸馏 | 五 |
| 对齐评估 | 对齐后模型 | HHH权衡点确认 | 红队 + benchmark + 内部评估 | 六 |
| 安全审计 | 上线候选模型 | jailbreak抗性 + 监管合规 | 红队、欧盟AI Act审计、SB 53 | 七 |

后训练把基础模型变成对话助手，是改造态的主体。RLHF是大厂标配但工程门槛高（几十人专门团队），DPO是开源社区主流但灵活性稍弱。合成数据（Constitutional AI、RLAIF、蒸馏式后训练）让后训练成本和扩展性大幅提升，但model collapse是隐忧。
...
大厂改造态团队（数据标注 + RLHF + 对齐 + 红队 + 合规）加起来几百人级别，跟base model训练团队规模相当。

## 8.2 用户反馈数据飞轮的局限：OpenAI 的盲点

中篇最有战略含义的一条判断在这里。

OpenAI那种"亿级用户每天产生数千万次thumbs up/down反馈"的数据飞轮，看起来是无可比拟的优势，实际效果有限。三个原因：

- 信噪比低。大多数用户懒得给反馈，给反馈的人未必懂，很多反馈是泄愤而不是评判
- sycophancy陷阱。用户喜欢"说我对、夸我聪明、措辞自信"的回答，反馈训练让模型自动收敛到拍马屁。GPT-4o 2024年4月那次"拍马屁版本"被全网吐槽紧急回滚就是这个机制翻车
- 场景偏移。日常聊天反馈多，编程、数学、长篇推理这类专业场景几乎没反馈

Anthropic走Constitutional AI这条反向打法（少量PhD级标注 + AI反馈）绕过了这三条结构性局限，在代码、长上下文、企业市场几个维度上2024-2025年陆续赶上或超过OpenAI。

但要hedge一下，"Anthropic超过OpenAI"这个说法过强，更准确是两家在不同维度互有胜负：

| 维度 | 当前领先方 |
|---|---|
| 代码生成 | Claude（SWE-bench、Cursor用户群体） |
| 长上下文理解 | Claude（1M token链路先行） |
| 企业 / 合规市场 | Claude（Constitutional AI路线） |
| 日常对话 / 多模态 | GPT（用户基数、DALL-E、Sora、语音） |
| Reasoning / 研究突破 | GPT（o1 / o3 / GPT-5.5路线） |
| 用户基数 / 生态 | GPT（ChatGPT、GPT Store、API渗透） |

这件事说明数据飞轮要成持续护城河，前提是"竞争对手收集不到类似数据"。当AI反馈能替代人类反馈时，这个前提就不成立了。

## 8.3 核心论点：感觉差距全来自改造态

中篇最核心的一个判断是，模型版本之间用户感受的差距，绝大部分源自改造态。用户感受到的"GPT-5比GPT-4强多少""Claude比GPT礼貌多少""新版变冷了"，几乎全部由后训练和对齐决定。

Base model升级周期3到6个月，每次能力跃迁一档。改造态迭代周期2到4周，每次调整就能影响用户体感。Base model是骨架，改造态是肌肉、皮肤、表情，决定产品最终长什么样。

这件事也解释了为什么不同厂家的模型读起来风格差异明显。Anthropic偏保守、OpenAI偏热情、Google偏中性，本质都是各家在HHH权衡点上选了不同位置。第三章3.4节那个GPT-5 2025年8月backlash的案例也是这个逻辑，OpenAI改了一次对齐权衡点，用户大规模反弹要求恢复旧版。

## 8.4 下篇预告：Agent 工程

下篇换条线，讲模型怎么从被动响应变成自主行动。

Agent工程是2025-2026最热也最被高估的方向之一。Cursor、Claude Code、Devin、Operator、Manus这些产品的差距，几乎完全在Agent工程层面，不在底层模型。下篇会拆这条线。
