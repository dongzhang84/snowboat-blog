...
上篇画了技术版图，中篇画了资本版图。到这一篇，问题落到最实的一层：这个赛道里，一个没有名人堂履历、也没有数亿美元的人或团队，到底还有没有位置，位置在哪，怎么进。

中篇的结论可以收成一句话。门是开的，但它分层，最上面那层够不着，下面那层够得着。造神层，也就是自建实验室、造"AI科学家"的那类公司，入场券是前OpenAI、前DeepMind级别的团队加数亿美元起步资金，对绝大多数人是橱窗里的样板。真正能进的是基础设施层，给科学Agent供底座、供评估、供工具、供数据的那一层。

这一篇就把这层的机会摊开，讲清三件事。第一，机会缺口具体在哪，需求侧是不是真的要。第二，什么样的背景在这层最有优势。第三，一个人从零起步，第一刀该切在哪、怎么走。全篇沿用上篇的简称AI4S。

# 第一章 机会缺口：哪一层还有普通人的位置
...
第一样是一致性判断。守恒律、对称性、量纲、单位、极限行为，这些是所有定量科学共享的硬约束。一个结果违不违反能量守恒、量纲对不对、在极限下退不退化到已知答案，受过物理训练的人一眼能看出来。第二样是不确定性量化。一个测量、一个预测到底有多可信，误差怎么传播，统计力学和机器学习本就数学同源，物理人对在高维、含噪空间里做推断有手感。

第三样是仿真与建模，把一个混乱的真实系统抽象成能算的模型、能跑的仿真，这是物理的看家本领，而且材料、气候、流体、天体用的是同一套建模思维。第四样是最优实验设计，给定有限预算，下一个最该测什么、算什么，这是贝叶斯实验设计和主动学习的地盘，也是物理和统计的交叉强项。这四样，每一样都能横切一大片科学Agent。
...
有背景的人这时最容易犯一个错，就是一头扎回自己最熟的学科，去做天体物理的评估、天体物理的工具。这没错，但它把人关进了一个很小的房间。
...
## 3.3 训练场与 verifier：资本最热的一块

这一类踩在当下最烧钱的地方，训练Agent。

现在训练推理和Agent模型，最主流的方法是给它可验证的奖励，业内叫RLVR。它的软肋是严重依赖一个能给出二元判定的验证器，在数学、代码这种有客观标准答案的地方好用，因为答案对错一跑就知道。一旦出了这个范围，多数开放式的科学问题根本没有现成的可靠验证器，你没法像判一道数学题那样，机械地判定一个新材料的性质预测或一个物理推导是不是对的。这个缺口被业内直接称作没人解决的验证器难题。
...
这一类吃的是最优实验设计这样资产，机会大、竞争少。

给定预算，下一个该测什么、该算什么，这是所有自驾实验室和数据采集Agent的大脑。它横切材料、生物、天文，因为每一类都要决定下一步做什么，而它又正好落在贝叶斯实验设计、物理和统计的交叉甜区。想想A-Lab那种一天要跑几十个样品的自动线，每一个样品该合成什么、下一批往哪个方向调，背后就是一个实验设计问题，选得好，同样的预算能多发现好几种材料，选得差就是烧钱空转。这块资本密集，自驾实验室正大把烧钱，却几乎没人把它单独抽出来做成一层通用设施，多数团队还是把它埋在各自的实验室里各做各的。上篇就点过，高维实验设计加不确定性管理，是物理背景者的天然甜区。

## 3.5 接口与复现：把仿真和验证接给 Agent

这一类偏工程，但同样吃学科判断。

一是仿真器的标准化Agent接口，把密度泛函、分子动力学、流体、N体这些求解器，包成干净、带物理报错、Agent能稳定调用的工具。这类接口的价值在于一次做对、所有人复用，今天各家团队各自往Agent上接求解器，接法五花八门、报错也不统一，Agent调用起来动不动就崩，谁能定出一套标准接口，谁就成了所有仿真密集科学Agent绕不过的一层。二是可复现验证与注入错误检测，做一个通用的判定，这个AI科学结果换个人、换一次能不能重现，以及能不能查出被人塞进去的错。可复现是科学的命根子，而AI的非确定性正在加剧这道危机。

## 3.6 哪几刀最锋利

菜单很长，如果只押少数几个，最值得押的是这三个，因为它们横切最广、最贴当下钱口、又最吃学科判断。

一是训练场那层的科学verifier库，踩在训练Agent这个最热的基础设施上，科学verifier又奇缺。二是最优实验设计中间件，自驾实验室的大脑，资本密集，横切所有要决定下一步的科学Agent。三是假科学检测器，最贴下一章那个科学界要盾牌的当下痛点，叙事也最抓人。至于评估和基准，它不再是唯一主菜，而是散落在上面几类里的一种用法，因为把眼界从做一个学科的基准放大到建横切的设施之后，verifier、实验设计、不确定性量化这些，市场更大，也更难被一个小团队抢先占满。
...
先看一类反复出现的失败。把AI做成替代专业人士的工具，结果遭到强烈抵触，项目做不起来。根源往往不在技术，在于定位站错了边。当一个产品被那个领域的人理解成要取代我、威胁我的饭碗、冒犯我这门手艺，本能反应就是抵触，无论技术多好。教育领域不少AI尝试就栽在这里，老师把它当成了替代者。

这个教训对AI4S极其重要，因为科学界对AI的态度，和教育界并没有本质不同。

## 4.2 科学界对 AI：又用又怕，而且怕得很具体
...
但同一批成果里就藏着怕。A-Lab那41种，很快被伦敦大学学院的材料化学家Robert Palgrave公开质疑，指出它的X射线衍射分析有严重问题，一部分所谓新材料其实是同一种物质或存在混相，真实数目远低于41，论文后来做了更正。
...
把这一章收成一条必须死守的红线，做盾牌，不做替代。

具体说，做帮科学家把关、验证、复现AI的工具，避免做替科学家做研究、抢科学家署名的系统。前者科学界欢迎、需求方在主动找，后者会重摔一次被专业群体集体抵触的跟头。这条线从产品第一天起就要守住，因为一旦站错，再好的技术也救不回来。
...
## 5.2 细分尚早：科学评估对纯 VC 还偏早

但要把话说全。LMArena评的是通用大模型，受众极广、商业化路径清楚。一旦细化到科学评估，给某个学科的科研Agent出考卷，受众一下子窄了很多。

现在托底这块的，主要还是政府和公益的钱。美国能源部2025年11月启动创世纪计划，后续发布约2.93亿美元招标。NSF和英伟达联手的OMAI项目约1.52亿美元。Google.org还有一个规模约3000万美元的AI科学资助计划。

这一水儿是政府和公益资本，几乎见不到纯商业VC的身影，本身就说明问题。换句话说，科学评估作为一项严肃的学术与基础设施工作是真实的，但作为一门纯商业的风投生意，眼下还偏早。这不是坏消息，它恰恰说明这块地还在被公共资金养着、等一个商业化拐点，先进来的人有的是时间把位置坐稳。

## 5.3 含义：先学术，后商业

大类已验证、细分尚早，这个组合恰恰指向一条最适合学科背景者的路径，学术先行，而不是一上来就纯VC创业。

理由很顺。科学评估现在更像一块先靠公益和学术养着、等商业化拐点的地。对学科背景者，这反而是好事，他们本来就在学术体系里，发一篇扎实的基准论文、把它开源、在领域里立起声誉，是天然擅长、风险又极低的动作。等这块地的商业化拐点到来，参照LMArena从一个学校项目走到约17亿美元估值的路，此时手里已经握着声誉和标准制定权，再谈服务化、商业化或被并购，都顺理成章。这条路径的具体走法，是下一章的主题。

# 第六章 入场路径：从一篇基准论文到一个事实标准
...
上篇说，AI正在从读科学走向做科学，范式转变是真的，但它做得还很不可靠，ScienceAgentBench约三成、PaperBench约两成的成绩就是证据。中篇说，资本已经认了这个方向，门是开的，钱在加码，但代价是慢，而且这扇门分层。下篇说，对一个有学科背景、却没有巨额资本的人，能进、也该进的，是基础设施层里那些只有懂科学的人才能做、巨头又懒得做的活，最好把学科判断当成一把横切所有科学的通用武器，去建那些任何科学Agent都要用的检验、验证、训练设施。

所以最后的选择，其实是一个身份问题，造神者，还是守门人。造神者要造出AI科学家，那是名人堂团队加数亿美元的豪赌，也是最可能先爆的雷。守门人不造神，他给这场造神运动当裁判、当验货员、当那面让科学界对抗AI泛滥的盾牌。守门人不需要数亿美元，靠的是别人翻不过去的学科判断力，他站在科学家这边，守的是所有定量科学的门，而不只是一个学科的门。

每一个AI自己设计出分子、自己合成出材料的高光背后，都需要有人问一句冷静的话，它真的做对了吗，还是只是看起来做对了。A-Lab那四十多种新材料后来被材料化学家逐一核查、打了不小的折扣，就是这句冷问的价值。能可靠回答它真的做对了吗的人，价值一点不比造神的人低，而且在未来很长一段时间里都会是稀缺的。对一个懂科学的人来说，这或许不是最耀眼的位置，但很可能是最扛得住、也最难被取代的位置。门开着，钱在加码，而最锋利的那一刀，恰好需要懂科学的人手里那把别人没有的尺子。
...
> 按主题归类，便于逐条核对。指标随版本更新可能变化，正式引用前请以一手来源为准。

**科研Agent能力与基准（第二、三章）**

- ScienceAgentBench（Chen et al., 2024, [arXiv 2410.05080](https://arxiv.org/abs/2410.05080)）：102个任务取自44篇论文，最强 agent 约32%，o1 自调试约42%。
- PaperBench（OpenAI, 2025, [arXiv 2504.01848](https://openai.com/index/paperbench/)）：复现20篇 ICML 论文，最佳 agent 约21%，ML 博士48小时约41%。
- ChemCrow（Bran et al., [Nature Machine Intelligence 2024](https://www.nature.com/articles/s42256-024-00832-8)）、Coscientist（Boiko et al., [Nature 2023](https://www.nature.com/articles/s41586-023-06920-w)）：自主化学实验 agent。
- RLVR 与验证器缺口（[arXiv 2506.14245](https://arxiv.org/abs/2506.14245)、VerifyBench [arXiv 2507.09884](https://arxiv.org/html/2507.09884)）。

**需求侧与研究诚信（第四章）**

- A-Lab 17天合成41种新化合物（[Nature 2023](https://www.nature.com/articles/s41586-023-06734-w)），及 Robert Palgrave 对其 PXRD 分析的质疑（[Chemistry World](https://www.chemistryworld.com/news/new-analysis-raises-doubts-over-autonomous-labs-materials-discoveries/4018791.article)）。
- arXiv 2025年10月底收紧 CS 综述与立场类投稿（[arXiv 官方博客](https://blog.arxiv.org/)、[404 Media](https://www.404media.co/arxiv-changes-rules-after-getting-spammed-with-ai-generated-research-papers/)）。
- ICLR 2026 约21%评审疑似 AI 生成，检测机构 Pangram Labs 估算（[Pangram Labs](https://www.pangram.com/blog/pangram-predicts-21-of-iclr-reviews-are-ai-generated)）。
- 对 Sakana AI Scientist 的独立评估，含表层评审与约42%实验代码报错（[arXiv 2502.14297](https://arxiv.org/abs/2502.14297)）。

**资本（第五章，详见中篇来源）**

- LMArena 累计约2.5亿美元、估值约17亿（[TechCrunch](https://techcrunch.com/2025/05/21/lm-arena-the-organization-behind-popular-ai-leaderboards-lands-100m/)、[Investing.com](https://www.investing.com/news/stock-market-news/ai-startup-lmarena-triples-its-valuation-to-17-billion-in-latest-fundraise-4433211)）。
- Axiom Math 种子约6400万、A轮约2亿（[SiliconANGLE](https://siliconangle.com/2026/03/12/verifiable-ai-startup-axiom-raises-200m-prove-ai-generated-code-safe-use/)）。
- 能源部创世纪计划约2.93亿招标（[energy.gov](https://www.energy.gov/articles/energy-department-announces-293-million-funding-support-genesis-mission-national-science)）、NSF 与英伟达 OMAI 约1.52亿（[NSF](https://www.nsf.gov/news/nsf-nvidia-partnership-enables-ai2-develop-fully-open-ai)）、Google.org AI 科学资助约3000万（[google.org](https://www.google.org/impact-challenges/ai-science/)）。

> 说明：这是一篇研究性综述的收官篇，覆盖范围基于公开检索，难免有遗漏。部分数字随融资与版本更新会变化，落稿前请逐条核验。
