...
这个转换说出来很轻，量级却不小。它意味着接下来AI的进步重心，从"模型再涨多少分"，转到"用现有模型能搭出多复杂的东西"。

Agent就是这个转向的典型代表。给它一个任务："帮我研究三家公司，下周给一份对比报告"，它自己去查公开资料、调用搜索API、调用计算工具、写代码、整理结果、最后写出一篇带数据带引用的报告。
