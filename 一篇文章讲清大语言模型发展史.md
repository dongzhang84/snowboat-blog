...
这些新模型算不算原创？要分层看。架构底座上，主流大模型基本都是decoder-only Transformer，也就是只用Transformer里负责生成的那半边，一路从左到右预测下一个token。这不是某家公司重新发明的东西，源头是Transformer和GPT路线。LLaMA的作用，是把这套路线做成了一个高质量、可研究、可复现的开放样板。

权重层面，Qwen、DeepSeek、GLM、Yi、Mistral、Grok、Gemini都不是LLaMA的衍生权重，更不是OpenAI模型的复制品。它们大多是自己收数据、自己训练、自己做后训练。换句话说，它们不算"抄模型"，但也不是从零发明一门新架构。

真正被大量借鉴的是架构细节和训练配方。位置编码解决的是"模型怎么知道词在句子里的位置"，今天常见的RoPE来自公开论文，被LLaMA采用后变成事实标准。归一化和激活函数解决的是"模型怎么稳定训练、怎么表达非线性"，RMSNorm、SwiGLU这类选择也来自公开论文，LLaMA把它们组合进了一个好用的配方。GQA/MQA解决的是"生成时怎么少存一点KV cache、推理更便宜"，它们属于attention层里的工程优化，不是模型能力的全部来源。

MoE是另一类东西，它改的是Transformer里的前馈网络部分。普通模型每个token都走同一套参数，MoE让不同token走不同专家，模型总容量变大，单次推理成本不跟着等比例变大。这条线主要来自MoE、GShard、Switch Transformer这些公开论文，后来被Mixtral和DeepSeek做成了开源前沿里的主流路线。

RLHF、DPO、RLAIF又不是基础架构，而是后训练和对齐方法。预训练让模型会续写，后训练让模型更会按人的指令回答。RLHF来自InstructGPT这条公开路线，RLAIF是Anthropic一类实验室推动的方向，DPO则是开源社区很快采用的更便宜做法。闭源的OpenAI和Anthropic给不出权重，但给出了方向感。早期开源社区还大量用GPT-4和ChatGPT生成指令数据做蒸馏。后来的前沿玩家，尤其Qwen和DeepSeek，已经不只是蒸馏别人，而是在数据、训练系统、MoE、后训练和推理能力上做自己的工程。
