...
Agent不是2024年才出现的概念。ChatGPT刚火的时候，大家已经在想一件事：如果模型能听懂人的话，能不能让它不只是聊天，而是接上软件、数据库、搜索、文件、支付、邮件、日历，替人完成一个任务？2023年3月，OpenAI推出ChatGPT plugins，让ChatGPT可以调用浏览器、代码解释器和第三方服务。几个月后，function calling进入API，开发者可以把一个外部函数描述给模型，让模型判断什么时候调用它。
...
所以，Agent基本可以看作SaaS的一次升级，但它不只是"给SaaS加一个聊天框"。传统SaaS是人点击按钮，软件执行确定流程。Agent化的SaaS，是人说目标，模型拆步骤，软件系统调用工具完成动作。CRM里，它可以自动整理客户背景、生成跟进邮件、更新字段。财务系统里，它可以查发票、对账、提醒异常。研发系统里，它可以读issue、改代码、跑测试、发PR。企业软件从"表单和按钮"往"目标和流程"迁移。
