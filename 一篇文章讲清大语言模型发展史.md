...
这些新模型算不算原创？要分层看。权重层面，Qwen、DeepSeek、GLM、Yi、Mistral、Grok、Gemini都不是LLaMA的衍生权重，更不是OpenAI模型的复制品。它们大多是自己收数据、自己训练、自己做后训练。

架构和方法层面，它们当然站在公开技术路线之上。底座是decoder-only Transformer，训练目标是next-token prediction，位置编码、归一化、激活函数、GQA/MQA、MoE、RLHF、DPO、RLAIF这些零件，来自Transformer、GPT、Chinchilla、LLaMA、Switch/GShard、InstructGPT等一串公开论文和开源实践。闭源的OpenAI和Anthropic给不出权重，但给出了方向感。早期开源社区还大量用GPT-4和ChatGPT生成指令数据做蒸馏。后来的前沿玩家，尤其Qwen和DeepSeek，已经不只是蒸馏别人，而是在数据、训练系统、MoE、后训练和推理能力上做自己的工程。
...
中国能起来，靠的是几件事一起发生。阿里、字节、腾讯、百度这种本土大厂能持续投入。中文互联网数据自成一池，给中国模型一个独立数据来源。美国对华GPU限制反而逼出DeepSeek那种工程效率，把MoE、FP8、强化学习配方压到更极致。"AI主权"叙事跟中国"必须自主"的政治逻辑天然契合。百模大战又逼出了开源竞争，DeepSeek、Qwen直接放权重，迅速在国际社区累积口碑。
