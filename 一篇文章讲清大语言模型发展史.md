...
ChatGPT之后两年，闭源前沿之外的玩家从美中欧三个方向集体入场，全球版图从OpenAI单极走向多极。

美国这边，Google反应慢了一拍但没缺席。2023年2月仓促发了Bard应对ChatGPT，效果一般，被嘲讽过几个月；2023年12月推出Gemini 1.0替换Bard，2024年2月Gemini 1.5（百万级上下文窗口），2024年12月Gemini 2.0。从2024年起，Google在多模态和长上下文这两块开始压住OpenAI。xAI（Elon Musk的新公司）2023年7月成立，2023年11月发Grok-1，2024年陆续发Grok-2、Grok-3，靠X平台的实时数据做差异化。

欧洲只剩Mistral一家。法国Mistral 2023年5月成立，几个月内开源Mistral 7B和Mixtral 8x7B（前面MoE那一节讲过）。三个创始人正好是LLaMA的"原班作者"：Arthur Mensch出自DeepMind，Guillaume Lample和Timothée Lacroix是Meta FAIR团队、参与训了LLaMA-1，等于带着技能出来另开一摊。Mistral跟LLaMA在架构家族上同源，但权重自己从零训。它的特色是高效，小参数量打出大效果，在欧洲拿到大量企业客户。
...
中国这边真正杀出来的是少数几家。智谱GLM系列在企业市场占住一席。阿里Qwen 2024年起在开源榜单上经常压过Llama，国际开发者社区开始大规模采用。Moonshot Kimi在长上下文这条线上做差异化。豆包靠字节App矩阵的入口做用户规模。2024年底DeepSeek V3发布（前面MoE那一节讲过）、2025年初R1发布，把开源前沿模型的成本和效果一起推到所有人都被迫重新算账的地步。这一波让海外圈第一次集体重视中国开源模型。

这些玩家大多数都不是LLaMA的衍生品。架构上几乎所有人（Mistral、Qwen、DeepSeek、Yi、Gemini、Grok等）都用decoder-only Transformer加类LLaMA的组件（RoPE、SwiGLU、RMSNorm、GQA），但权重都是自己从零训的。Google Gemini跟LLaMA没继承关系，它有自己的PaLM血脉。中国的文心、ChatGLM都比LLaMA出现得早，架构独立。Mistral是特殊例子，创始人就是LLaMA-1作者，"血缘"最近，但商业上仍是独立训练。LLaMA真正贡献的是证明了"中等参数 + 大量数据 + 开放权重"的训练配方走得通，各家自己用这套方法跑，不是大家拿LLaMA权重微调。

到2025年，全球大模型格局基本是中美两极。美国占闭源前沿（OpenAI / Anthropic / Google / xAI），中国占开源前沿（DeepSeek / Qwen / GLM / Yi），欧洲只剩Mistral一家在第二梯队，其他地区（日本、韩国、印度、中东、东南亚）基本没有前沿模型。

中国能起来，靠的是几件事一起：阿里、字节、腾讯、百度这种千亿级营收的本土大厂能持续往大模型上烧几亿到几十亿美元；中文互联网数据自成一池，给中国模型一个独占数据来源；美国对华GPU限制反而逼出DeepSeek那种工程效率（MoE、FP8、强化学习配方），用更少算力做更多事；"AI主权"叙事跟中国"必须自主"的政治逻辑天然契合；百模大战逼出了开源竞争，DeepSeek、Qwen直接放权重，迅速在国际社区累积口碑。

欧洲没起来的原因正好相反：没有Google / Meta / 阿里 / 腾讯这种千亿规模本土大厂兜底，Mistral总融资6亿多美元，跟OpenAI几百亿、Anthropic几十亿没法比；EU AI Act 2024年通过给前沿模型加了一堆合规成本；欧洲顶级AI研究员大批被美国大厂挖走，本地留不住人；欧洲多语言多国家，市场分散，没有中国那种统一14亿用户的大市场反哺模型。

其他地区情况类似欧洲。日本Sakana AI 2024年涨了一阵，但定位"小而美"不冲前沿；韩国LG、Naver各有模型，没出圈；印度和中东主要是建数据中心租给美国玩家。这些地区2025年的角色更接近"模型用户"而非"模型生产者"。

整个格局基本对应全球互联网版图：闭源前沿在美国，开源前沿在中国，欧洲拿1.5个名额，其他都是参与者。
