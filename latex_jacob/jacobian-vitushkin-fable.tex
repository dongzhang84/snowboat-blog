...
determinant $-2$ was announced. The map is not injective, so it gives a
counterexample to the Jacobian Conjecture in every dimension $n\ge 3$. Its
construction is very close to an obscure 1999 Russian paper of Vitushkin.
This note writes out, step by step, one way to derive the three-dimensional
counterexample from Vitushkin's two-dimensional example, and shows that when
the extra dimension degenerates, the construction falls back to the
...
constant $-2$, but sends three distinct points to the same image.
...
The construction is very close to work of the Soviet/Russian mathematician Vitushkin in a 1999
Russian-language paper. That paper used $\sigma$-processes to build
two-dimensional rational maps with constant Jacobian that are globally
non-invertible, getting very close to the later three-dimensional example.
Because it was written in Russian and was not widely read, the connection
seems to have been missed.

This note has one purpose: to write out, step by step, the derivation chain
...
polynomials. It also shows that when the new dimension degenerates, the
construction collapses back to the two-dimensional prototype. In this sense,
the two examples look like two slices of the same construction, rather than
...
% =====================================================================
The basic point of a counterexample is that several inputs can land on the
same output. So we ask the reverse question: fix an output; which inputs map
to it? These preimages are exactly the roots of a certain equation in an
auxiliary variable $T$ --- the \emph{fiber equation} (the equation for the
preimages of a fixed output). This equation is not arbitrary; it keeps track
of how many preimages there are and where they sit.
...
\end{aligned}
\]
with $\Jdet(P,Q)=2y$. The $\sigma$-process is not just an arbitrary algebraic
manipulation. It is a choice of coordinate at infinity that exactly cancels the
...
\]
Why this $t$? Because it carries the same kind of pole as the stubborn $1/y$
in Vitushkin's example: the denominator $1/x$ is packed inside the root,
carried by the root rather than exposed in the map's formulas.
...
at $x=\infty$: the bad behavior has been pushed to infinity, and the finite
part is a clean three-sheeted cover (which is what makes a constant nonzero
...
This step matches the two coordinate systems by a three-dimensional
birational change.
...
one-way derivation shows the two constructions are closely related.
...
This point lies exactly on the two-dimensional slice. Its third output
...
dimension adds. The special fiber is a concrete instance of ``3D $=$ 2D plus one
...
Russian paper, seems to provide the larger part of the underlying mechanism,
for concrete and checkable reasons.
\begin{enumerate}[label=\textbf{\arabic*.},nosep,leftmargin=*]
  \item The whole machine is his. The $\sigma$-process --- the
  ...
  \item The core local model is almost verbatim his. Vitushkin's
  ...
  \item The mechanism is his. ``Constant nonzero Jacobian $+$
  ...
  \item He located the obstruction. His Proposition~1 (a
  ...
  \item He was one step from the finish. Method, a usable local
        model, the mechanism, the obstruction diagnosis --- he had all four;
        what remained was ``lift to three dimensions and balance the
        coefficients.''
\end{enumerate}

What Fable adds is the last step that matters: choosing a new variable $z$ to
...
write the framework; finding the specific balanced example is the hard part.

An important delimitation: saying Vitushkin has $60$--$70\%$ refers to the
...
This note gives a concrete derivation linking the two examples (the $c=0$
collapse, the term-by-term match of local models), which is a checkable
mathematical claim. Whether Fable actually retrieved and used the Russian
paper is a question of provenance that mathematics cannot settle, and is not
claimed here.
...
and Fable's three-dimensional counterexample related? Yes, and the relation
is explicit. The whole chain is: use Vitushkin's $\sigma$-process to
...
force the branch detector $\Phi'(t)$ to a pole~(3), pushing branching to
...
coordinate). The two examples are more than lookalikes: one is the
two-dimensional slice, and the other is the three-dimensional version of the
same construction.

A few caveats. First, ``why exactly this $\sigma$-chain'' is taken
...
twenty-year-old two-dimensional near-miss forward by one decisive step; the
