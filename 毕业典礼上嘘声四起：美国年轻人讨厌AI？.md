...
人的问题的部分，路径要复杂得多。一个挺好用的工作模式是"人在两头，中间是AI"：创造的源头是人，最终把关也是人，中间大量重复劳动交给AI做。一个负责任的内容平台不应该让AI自由生成内容直接发出去，而应该是人提出题目和方向（前端把关）、AI做素材整理和初稿、再由人审核改写和发布（后端把关）。AI客服可以处理80%的简单问题，但每个客户都有一个一键转人工的按钮，疑难case第一时间转给人。AI朗读名字之前，先让一个真人测试一遍每个名字的发音，确认无误再上线。

这种"两头是人，中间是AI"的模式不省最多的钱，但它把AI用在它擅长的中间环节（速度、规模、重复任务），把人留在AI不擅长的两端（判断、品味、责任）。它在很多场景下其实是用户体验最优的方案。
...
四层反感都是真实的。但每一层真正能解决问题的方向都落在具体的制度修补上：失败的劳动政策（生计层）、过时的教育评估制度（学校层）、过时的版权和补偿法（尊严层）、平台问责的缺失（消费层）。这些才是AI背后真正应该被推动的事。
