...
这是 AI 最老牌的那一块，也是其他所有方向的地基。你手上有一堆表格数据，比如用户画像、交易记录、传感器读数，希望机器从这些数据里学出规律来预测一个结果或者把样本归类，这就是传统 ML 要干的活。经典方法一路数下来就是线性回归、逻辑回归、决策树、随机森林、XGBoost 这几家，底层吃的是线代、概率统计和凸优化那三板斧。

在工业界这一块今天依然遍地跑。风控评分、保险定价、信用模型、医疗诊断预测、搜索排序的底层、时间序列预测，这些场景里"谁也打不过 XGBoost 加特征工程"是行话。但在 2026 年的风口上它不是主角。你学这一块主要为了打底，真正把它当职业主线的人在减少。
