...
下面这 10 个由浅入深的问题，是我作为启发者给你埋下的十级台阶。每一个都去和 Claude 或 ChatGPT 聊，聊到你能用费曼法把答案讲给不懂的朋友听为止。一层走完再上下一层。

1. 你打开 ChatGPT 和模型聊天的时候，背后到底在哪里运行？服务器在哪里，你的电脑在做什么，它又在做什么？
2. 如果我不用 ChatGPT 的网页，而是想让自己写的程序直接跟模型聊天，中间要搭起什么东西？这就是 API 的作用吗？
3. API 这个词在 AI 出现之前就存在了，它在别的场景里一般是干什么的？为什么这个老概念能被 AI 时代自然借用？
4. 一次 API 调用里，你的程序和服务器之间传递的到底是什么？文字能直接发过去吗？为什么要打包成某种格式？
5. 在代码里调用 API 要用一门编程语言。为什么 Python 成了事实上的默认选择？如果我不会 Python，能先把 API 调通吗？
6. 当我让模型连续回答几个问题时，它其实"记得"我之前说过什么吗？如果不记得，想做多轮对话该怎么办？
7. 调用 API 是要花钱的，按什么计算？一分钟？一个字？背后的计价逻辑是什么？
8. 如果我一次发一段很长的文字给模型，它能全看懂吗？有没有"消化量"的上限？超过上限会发生什么？
9. 我怎么判断 AI 给我的回答质量好不好？除了"看起来合理"，有没有办法把它变成可以量化的指标？
10. 等我能调通一次 API 之后，下一步通向哪里？RAG、Agent、Fine-tuning 这些词分别意味着什么，什么时候该用？

把这 10 个问题一级一级问下来，你对这一层需要的知识就有了完整的框架。剩下的就是把它沉淀到手上，配合三个项目做完。
