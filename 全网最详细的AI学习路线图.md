...
但是有两个词你必须懂：precision 和 recall。不需要回头翻大部头的统计学教材，用白话说一遍就够。

Precision（精确率）问的是：你给出的所有答案里，多少是对的？返回 10 条结果，7 条相关，precision 就是 70%。

Recall（召回率）问的是：所有本该被找回来的相关答案里，你找回来了多少？真实相关的文档有 10 条，你只捞出了 7 条，recall 就是 70%。

这两个指标天然冲突。想保险就多返回候选，recall 上去了 precision 会掉；想精准就少返回几条，precision 上去了 recall 会掉。一套 RAG 系统调的就是这对 tradeoff。
