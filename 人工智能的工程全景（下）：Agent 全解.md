...
人工智能的工程全景系列，[上篇](https://x.com/snowboat84/status/2061962883651731602)讲了AI工程栈的运行态：硬件、电力、训练、推理。[中篇](https://x.com/snowboat84/status/2065215177029787705)讲了改造态：推理优化速成、后训练、对齐、安全。中篇结尾我留了一句话，说Cursor、Claude Code、Devin、Operator、Manus这些产品的差距几乎全在Agent工程层，不在底层模型。下篇就来兑现这句话。
...
工具调用，让模型能搜索、能跑代码、能调外部API，这是第二章。规划，把一个复杂目标拆成子步骤，这是第四章。记忆，跨步骤、跨会话保留状态，这是第五章。自我纠错，发现错了能重试，这一条没有独立成章，融在第七章的可靠性工程里。终止，知道什么时候该收工、什么时候该放弃，这是最容易被忽视也最难做对的一条，模型常常不知道自己已经完成或已经无解，会一直转下去烧钱。可观测，跑过的每一步都能回溯，这是第七章的最后一节。

这六条里，模型本身只直接负责其中一部分，剩下的全靠模型外面那层工程兜底。这正是本篇的主题。

# 二、Tool Use 与 MCP 生态

工具调用是Agent的第一个核心能力。模型本身只会生成文本，要它去搜网页、跑代码、查数据库，全靠工具。这一章拆工具调用的工程演进，重点是2024年底MCP协议带来的那次标准化。
...
网页搜索是最常见的一类，给模型一双能看实时互联网的眼睛，常用的有Brave、Exa、Perplexity这些搜索API。代码执行让模型能真的跑一段程序验证结果，关键是要放进沙箱里隔离，不然模型生成的代码直接在你机器上跑是灾难。文件系统工具让模型能读、能写、能grep，这是Claude Code这类编程Agent的命脉。数据库查询让模型能发SQL、能查向量库，把企业自己的数据接进来。

浏览器操控是更重的一类，用Playwright、Browser Use这类工具让模型像人一样点页面、填表单，第六章会专门讲它有多难。还有截图解析，用视觉模型加OCR让Agent看懂屏幕上的图形界面。这几类工具的难度天差地别，搜索和文件系统已经相当成熟，浏览器和桌面操控到今天还在和可靠性死磕。
