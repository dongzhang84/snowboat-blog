...
| 端到端"AI科学家" | AI Scientist v1/v2、Kosmos、Agent Laboratory、SciAgents、Curie、Carl | 早中期（跑通≠真发现） |
...
|---|---|---|---|
| 科学基础模型SciFM | 提供有科学素养的推理底座 | BioNeMo、FM4M、气候时序模型、深度势能 | 低（重训练、重数据） |
| 自驾实验室 | 把决策变成真实物理操作 | Coscientist、A-Lab、通用编排层（待补） | 低（重资产） |
| 评估 / 基准 / 可复现 | 验收与可信度 | ScienceAgentBench、PaperBench、LABBench2及物理天体系列 | 高（学科专家可轻装切入） |
| 工具与编排 | 连接外部世界、协调多智能体 | agent框架、科学数据库接口、仿真接入 | 中 |
...
回顾上篇：我们厘清了科研Agent的概念（第二章），用纵切的六个环节（第三章）与横切的四层基础设施（第四章）画出了完整的技术版图，扫描了五个学科的分布与逻辑（第五章），并给出了一个诚实的成熟度判断，能力强、可靠性弱、真实新发现尚早（第六章）。

可以确认的是，**AI for Science的版图已经成形。** 它已经从一个模糊的口号，长成了一个有清晰环节、清晰基础设施层、清晰学科分布、且有大量可查证工作的真实领域。范式转变是真的，AI确实正在从"读科学"走向"做科学"，尽管"做"得还很不可靠。
