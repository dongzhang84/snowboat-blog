...
最后一章讲我（作者）认为下一步对的路线。需要先说清楚定位：这一章不是描述行业正在做的事，是一个提议，跟第七章那个"事后总结的方法论"性质相似。如果将来真有人按这条路做出来，再回头看这一章可能是历史的注脚。如果没有，这一章是个一直没被兑现的判断。
...
事实是Transformer在大多数场合表现强，但有几个场合它并不是最优选择。长序列建模有更高效的state-space models。分子和晶体等科学数据需要等变网络。连续物理系统的生成上扩散模型有难以替代的优势。决策和控制问题Hamilton系统族的能量守恒结构更稳定。结构化表格数据上基于决策树的XGBoost、LightGBM至今没被神经网络打败。这些不是个别例外，是系统性的现象。
