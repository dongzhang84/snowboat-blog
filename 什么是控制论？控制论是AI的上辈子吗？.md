...
控制论死了吗？没死。它被肢解后分别接进了AI、信息论、控制工程、系统科学、认知科学这些学科的地基里。控制论作为名字消失了，作为思想活在每一行梯度下降的代码里。

本文标题说"控制论是AI的上辈子"。"上辈子"是个隐喻，严格说控制论没死过。AI是控制论拿到算力之后最大的一次实现。这篇文章证明的就是这件事。
...
Maxwell做的事是把整个"调速器+蒸汽机"系统写成一组耦合的常微分方程。变量包括蒸汽机的角速度、调速器小球的位置、阀门开度、蒸汽进入量。这些量之间互相影响：速度变化导致小球外抛，小球外抛带动阀门关小，阀门关小蒸汽进入量减少，蒸汽减少又使速度变化。整个系统在数学上是一个闭环。

接下来他做了一个标准化的简化叫**线性化**（linearization）。意思是：系统在某个稳定工作点附近，把方程在那一点做泰勒展开，只保留一次项。结果是一个线性常微分方程，比原来的非线性方程好处理。这是控制理论里至今还在用的一招，所有稳定性分析的起点几乎都是先线性化。

然后是关键一步。假设解的形式是e^(λt)（指数函数），代入线性方程得到一个关于λ的多项式方程，叫**特征方程**（characteristic equation）。这个方程的根λ决定了整个系统的行为：

- λ是负实数：扰动指数衰减，系统稳定
- λ是正实数：扰动指数发散，系统炸掉
- λ是复数a+bi：扰动是e^(at)·cos(bt)的形式，振荡+增长/衰减。实部a决定振荡幅度是越来越大还是越来越小，虚部b决定振荡频率

Maxwell由此得出一条判据：调速器稳定的充要条件，是特征方程所有根的实部都必须是负数。负实根意味着扰动衰减，复根的负实部意味着振荡幅度也衰减。

这条判据当时直接解释了工程师们的困惑。机器为什么会震荡？因为特征方程出现了正实部的复根。设计调速器时怎么避免震荡？调参数让所有根的实部都跑到负半平面。

Maxwell本人只解决了3阶及以下的系统。5阶以上的特征方程要判断根的实部正负，符号计算变得很复杂。这个问题后来由英国数学家Edward Routh 1877年解决（《A Treatise on the Stability of a Given State of Motion》），独立地瑞士数学家Adolf Hurwitz 1895年又解决了一遍。两个解法被合并称为今天教科书里的**Routh-Hurwitz稳定性判据**：给你任意阶的特征方程，不用求根，直接看系数矩阵就能判断稳定性。

这是反馈第一次被数学化。Maxwell这篇论文出版后被忽视了80年，因为论文很难读，加上19世纪末调速器的工程问题没那么严重，工程界用经验凑过来了。1948年Wiener在《Cybernetics》里专门提了这篇文章，说它是控制论这门学科真正的源头。从那之后所有控制理论教材的第一章都从Maxwell 1868开始讲起。
...
1940年11月MIT数学教授Norbert Wiener给一群伺服工程师（servo-engineers）做了个建议。用某种特定频响的网络可以预测飞机的未来位置，从而提高高射炮命中率。

背景是二战的防空。飞机飞得快，从地面发现到炮弹打到那个高度需要10秒到20秒，等炮弹飞到飞机早就飞走了。所以高射炮必须"提前量"瞄准，预测20秒后飞机在哪里。
...
这个项目战时没做出能用的硬件，但Wiener的思考从中浮出一个核心判断：有目的的行为，本质就是负反馈。打飞机要瞄前一点，是不断用"实际飞了哪"去修正"以为飞机会在哪"。猫扑老鼠，是用"老鼠当前位置"修正"猫扑出去的方向"。人伸手拿杯子，是用"手当前位置vs杯子位置的差距"修正肌肉指令。所有"目的论"都可以用反馈回路解释。

这个判断1943年写成了一篇19页的小论文《Behavior, Purpose and Teleology》，作者是Wiener+Bigelow+哈佛医学院的墨西哥生理学家Arturo Rosenblueth。这是控制论这一支的起点文献。

## 2.2 神奇的1943年：神经元和反馈的双生子
...
会议形式是跨学科同桌。数学家、神经生理学家、工程师、人类学家、心理学家、社会学家被强行放在同一间屋子里讨论"反馈"和"循环因果"这两个概念在各自领域是什么。第一次开会时大家几乎没有共通语言，物理学家听不懂人类学家在讲什么，反过来也一样。整整10届会议磨下来，磨出了一套跨学科的共同词汇。

这套词汇就是1948年Wiener拿去命名一门新学科的素材。
...
把这份清单按"哪个器官最完整地继承了控制论的肉身"排序，答案是控制工程，AI排第二。

控制工程藏在工学院里。今天给你家空调、汽车、飞机、火箭、生产线写控制算法的工程师，他们用的数学跟Wiener 1948那套是连续的。课本一直在更新，但PID控制器、状态空间方法、卡尔曼滤波这些东西的根都在1940-60年代的控制论里。这一支的特点是低调，工程界不喜欢用cybernetics这个炫词，就叫control。它最硬核、最完整地活下来了。

那AI呢？AI是控制论最大的孩子。它继承了控制论里"反馈+学习"的核心动作，加上了控制论从没有过的两样东西（算力和representation learning），长得比母亲大得多。但AI圈子今天基本不提自己的cybernetics血缘。大多数AI工程师没读过Wiener那本书，不知道梯度下降跟恒温器是同构的。AI是那种忘了母亲名字的孩子。
...
把7.4收一句。AI继承了控制论的骨架，也继承了它的天花板。这堵墙有多高、会不会被突破，是AI接下来十年的核心问题之一。

## 7.5 新一轮回魂：Friston的自由能原理
...
"上辈子"暗示死了又重生，是reincarnation。这个比喻在SEO和传播上很punchy，但严格说不对。控制论没死过。它作为名字消失了，作为思想是AI的活地基。每次你跑梯度下降，本质上跑的还是Wiener 1948年那套数学。每次你看agent收reward调整policy，本质上是Bellman 1957年那个方程。每次AI圈讨论alignment和reward hacking，本质上是在跟控制论70年前留下的结构性问题再较一次劲。

更准确的说法是：AI是控制论拿到燃料、补上representation learning之后的最大一次实现。"上辈子"这个比喻把它当成来世，丢掉了"同一个学科还活着"这层意思。

是不是只有一次实现？不一定。Friston这条线如果跑通，会是控制论的另一次实现，用另一套数学外壳。控制论这门学科作为名字会不会再回来？大概率不会，名字消失了就消失了。但作为思想，它会反复回到AI的每一个新阶段。
