...
上篇讲了AI工程栈的运行态：硬件、电力、训练、推理。中篇讲了改造态：推理优化速成、后训练、对齐、安全。中篇结尾我留了一句话，说Cursor、Claude Code、Devin、Operator、Manus这些产品的差距几乎全在Agent工程层，不在底层模型。下篇就来兑现这句话。
...
2024年12月19日，Anthropic发了一篇《Building Effective Agents》，给出一个到今天还被反复引用的区分：Workflow和Agent不是一回事。
